...
def connect_db():
    """Connect to the phpMyAdmin database (can be accessed anywhere)"""
...
class User:
    ...
    Req params: name, username, email"""
...
def load_user(id):
    ...
    if result is not None:
        return User(
            result["id"], result["username"], result["first_name"], result["last_name"]
        )
...
def homepage():
    ...
    return render_template(
        "homepage.html.jinja", now=datetime.now(), user=user, empty=empty
    )
...
def browse(page):
    ...
    add_range(
        "admission_rate", "admit_min", "admit_max", 0.01
    )  # Admission rate is stored as decimal
    ...
    if (
        length > 5 and page >= length - 2
    ):  # Only adjust if there are enough pages to shift
...
def update_user():
    ...
    # Flash error in the case of error
        ...
        flash(
            f"An error occurred while updating your settings! Error: {str(e)}", "error"
        )
        print(e)
